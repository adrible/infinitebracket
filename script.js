...
function setScreen(screen) {
  ...
  document.getElementById("pageTitle").textContent = titles[screen] || "Torneio Arena";
}
...
function renderTeams() {
  ...
  if (state.teams.length === 0) {
    list.innerHTML = `<p>Nenhum time adicionado ainda.</p>`;
    return;
  }
  ...
  state.teams.forEach((team, index) => {
    ...
    row.innerHTML = `
      ...
      <button class="remove-btn" data-remove="${index}">Remover</button>
    `;
...
function chooseGameType(teamA, teamB) {
  ...
  if (diff >= 25 && r < 0.18) return "blowout";
  if (r < 0.012) return "crazy";
  if (r < 0.20) return "open";
...
function totalGoalsForType(type, realism) {
  const profiles = {
    realistic: {
      tight: [{ value: 0, weight: 12 }, { value: 1, weight: 38 }, { value: 2, weight: 36 }, { value: 3, weight: 14 }],
      normal: [{ value: 1, weight: 15 }, { value: 2, weight: 35 }, { value: 3, weight: 32 }, { value: 4, weight: 18 }],
      open: [{ value: 3, weight: 20 }, { value: 4, weight: 34 }, { value: 5, weight: 26 }, { value: 6, weight: 15 }, { value: 7, weight: 5 }],
      blowout: [{ value: 4, weight: 32 }, { value: 5, weight: 34 }, { value: 6, weight: 22 }, { value: 7, weight: 9 }, { value: 8, weight: 3 }],
      crazy: [{ value: 7, weight: 28 }, { value: 8, weight: 28 }, { value: 9, weight: 20 }, { value: 10, weight: 14 }, { value: 11, weight: 7 }, { value: 12, weight: 3 }]
    },
    normal: {
      tight: [{ value: 1, weight: 26 }, { value: 2, weight: 38 }, { value: 3, weight: 26 }, { value: 4, weight: 10 }],
      normal: [{ value: 2, weight: 28 }, { value: 3, weight: 32 }, { value: 4, weight: 26 }, { value: 5, weight: 14 }],
      open: [{ value: 4, weight: 25 }, { value: 5, weight: 30 }, { value: 6, weight: 25 }, { value: 7, weight: 14 }, { value: 8, weight: 6 }],
      blowout: [{ value: 4, weight: 25 }, { value: 5, weight: 32 }, { value: 6, weight: 25 }, { value: 7, weight: 13 }, { value: 8, weight: 5 }],
      crazy: [{ value: 8, weight: 20 }, { value: 9, weight: 25 }, { value: 10, weight: 25 }, { value: 11, weight: 18 }, { value: 12, weight: 12 }]
    },
    chaotic: {
      tight: [{ value: 1, weight: 18 }, { value: 2, weight: 28 }, { value: 3, weight: 30 }, { value: 4, weight: 24 }],
      normal: [{ value: 3, weight: 25 }, { value: 4, weight: 28 }, { value: 5, weight: 25 }, { value: 6, weight: 15 }, { value: 7, weight: 7 }],
      open: [{ value: 5, weight: 22 }, { value: 6, weight: 25 }, { value: 7, weight: 24 }, { value: 8, weight: 17 }, { value: 9, weight: 12 }],
      blowout: [{ value: 5, weight: 20 }, { value: 6, weight: 26 }, { value: 7, weight: 25 }, { value: 8, weight: 18 }, { value: 9, weight: 11 }],
      crazy: [{ value: 9, weight: 18 }, { value: 10, weight: 22 }, { value: 11, weight: 22 }, { value: 12, weight: 20 }, { value: 13, weight: 11 }, { value: 14, weight: 7 }]
    }
  };
...
function simulateExtraTime(teamA, teamB) {
  ...
  const favA = teamA.power >= teamB.power;
  return favA ? { goalsA: 2, goalsB: 0 } : { goalsA: 0, goalsB: 2 };
}
...
function renderBracket() {
  ...
  state.bracket.forEach((round, roundIndex) => {
    ...
    round.forEach((match, matchIndex) => {
      ...
      const score = match.goalsA !== undefined ? `${match.goalsA} x ${match.goalsB}` : "x";
      ...
      card.innerHTML = `
        ...
        ${match.winner ? "" : `<button class="btn btn--ghost" data-simulate="${roundIndex}:${matchIndex}">Simular</button>`}
      `;
...
document.querySelectorAll("[data-screen], [data-screen-button]").forEach((el) => {
  el.addEventListener("click", () => {
    setScreen(el.dataset.screen || el.dataset.screenButton);
  });
});
...
document.getElementById("loadSuggested").addEventListener("click", () => {
  syncConfig();
  state.teams = suggestedTeams.slice(0, state.config.teamCount).map((team) => ({
    id: crypto.randomUUID ? crypto.randomUUID() : String(Date.now() + Math.random()),
    ...team
  }));
  renderTeams();
...
document.getElementById("simulateAll").addEventListener("click", () => {
  while (state.bracket.length > 0) {
    ...
    if (state.bracket[state.bracket.length - 1].length === 1 && state.bracket[state.bracket.length - 1][0].winner) {
      break;
    }
  }
